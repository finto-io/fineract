...
class SdkSetLoanStatusUseCaseTest {
    ...
    private CheckBalanceForCloseLoanValidator checkBalanceForCloseLoanValidator;
    ...
    void setUp() {
        ...
        checkBalanceForCloseLoanValidator = control.createMock(CheckBalanceForCloseLoanValidator.class);
        ...
        useCase = SdkSetLoanStatusUseCase.builder()
                ...
                .checkBalanceForCloseLoanValidator(checkBalanceForCloseLoanValidator)
                .build();
    }
     ...
     */
    @Test
    void setLoanStatus_close_success() {
        ...
        Call<PostLoansLoanIdTransactionsResponse> responseCall = control.createMock(Call.class);
        ...
        PostLoansLoanIdTransactionsRequest request = PostLoansLoanIdTransactionsRequest.builder()
                .locale(LOCALE)
                .build();

        expect(context.loanApi()).andStubReturn(loansApi);
        expect(context.loanTransactionApi()).andStubReturn(loanTransactionsApi);
        ...
        expect(loanIdStatus.getValue()).andReturn("Active");

        expect(loanStatusValidator.validateStatusChange("Active", loanStatus))
                .andReturn(true);
        checkBalanceForCloseLoanValidator.validate(loanResponse);
        expect(loanMapper.toRequestWithDateForClose(loanStatus)).andReturn(request);
        expect(loanMapper.toCommandDto(loanStatus)).andReturn(io.finto.integration.fineract.dto.enums.LoanStatus.CLOSED);

        expect(loanTransactionsApi.executeLoanTransaction(1L, request, "close")).andReturn(responseCall);
