...
public class SdkUpdateCustomerIdentifierUseCaseTest {
    ...
    void updateCustomerIdentifiers_AddNewIdentifierAndDeleteOldIdentifier(){
        var oldIdentifiers = List.of(createIdentifier(1, NATION_ID_CODE_NAME, "old_nation_id"));
        ...
        expect(identifierUseCase.createCustomerIdentifier(customerId, NATION_ID_CODE_NAME, "new_nation_id")).andReturn(null);
    ...
    void updateCustomerIdentifiers_theSameIdentifier(){
        var oldIdentifiers = List.of(createIdentifier(1, NATION_ID_CODE_NAME, "old_nation_id"),
                createIdentifier(2, PASSPORT_CODE_NAME, "old_passport"));

        expect(context.clientIdentifierApi()).andStubReturn(clientIdentifierApi);
        expect(clientIdentifierApi.retrieveAllClientIdentifiers(customerId.getValue())).andReturn(call);
        expect(context.getResponseBody(call)).andReturn(oldIdentifiers);
        expect(identifierUseCase.createCustomerIdentifier(customerId, DRIVER_ID_CODE_NAME, "new_driver_id")).andReturn(null);
    ...
    void updateCustomerIdentifiers_addAll(){
        ...
        expect(context.getResponseBody(call)).andReturn(oldIdentifiers);
        expect(identifierUseCase.createCustomerIdentifier(customerId, NATION_ID_CODE_NAME, "new_nation_id")).andReturn(null);
        expect(identifierUseCase.createCustomerIdentifier(customerId, PASSPORT_CODE_NAME, "new_passport")).andReturn(null);
        expect(identifierUseCase.createCustomerIdentifier(customerId, DRIVER_ID_CODE_NAME, "new_driver_id")).andReturn(null);
